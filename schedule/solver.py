...
class ScheduleSolver:
    ...
    def __init__(
        ...
        config: MonthConfig,
        post_preferences: dict[str, list[str]] | None = None,
            ...
            for d in self.config.days:
                ...
                for p, post in enumerate(self.posts):
                    ...
                    if p in self._24h_pidxs:
                        ...
                        hospital_years = (
                            emp.hospital_years
                            if emp.hospital_years
                            else emp.seniority
                        )
                        if self.seniority_filter and hospital_years < 5:
                            can_full = False
        ...
        for d in self.config.days:
            for p, post in enumerate(self.posts):
                ...
                if p in self._24h_pidxs:
                    ...
                    if len(day_emps) < S:
                        print(f"  ⚠  {post.name} день {d}: "
                              f"дневное покрытие {len(day_emps)}/{S}")
                    if len(night_emps) < S:
                        print(f"  ⚠  {post.name} день {d}: "
                              f"ночное покрытие {len(night_emps)}/{S}")

                    all_day = f_vars + d_vars
                    all_night = f_vars + n_vars
                    if all_day:
                        self.model.add(
                            sum(all_day) == min(S, len(day_emps)))
                    if all_night:
                        self.model.add(
                            sum(all_night) == min(S, len(night_emps)))
                else:
    ...
    def _post_preference_penalty(self):
        for e, emp in enumerate(self.employees):
            ...
            if not prefs:
                continue
            pref_rank = {pid: rank for rank, pid in enumerate(prefs)}

            for d in self.config.days:
                for p, post in enumerate(self.posts):
                    rank = pref_rank.get(post.id, len(prefs))
                    if rank == 0:
                        continue
                    w = rank * 15
                    if p in self._24h_pidxs:
